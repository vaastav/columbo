...
var (
	...
	reDMAIssue  = regexp.MustCompile(`main_time = (\d+): nicbm: issuing dma op (read|write) addr (0x[0-9a-fA-F]+) len (\d+) pending (\d+)`)
	reDMAExec   = regexp.MustCompile(`main_time = (\d+): nicbm: executing dma op (read|write) addr (0x[0-9a-fA-F]+) len (\d+) pending (\d+)`)
	reDMAComp   = regexp.MustCompile(`main_time = (\d+): nicbm: completed dma (read|write) op (read|write) addr (0x[0-9a-fA-F]+) len (\d+)`)
